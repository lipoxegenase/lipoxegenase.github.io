#!/usr/bin/env python3
"""
KatalystVC Excel API Microservice (Secure Version)
...
def setup_credentials():
    """Setup credentials securely if .env file doesn't exist."""
    env_file = '.env'
    
    if not os.path.exists(env_file):
        print("Setting up secure credentials for the first time...")
        print("This information will be stored in a .env file and should be kept secure.")
        print()
        
        smtp_server = input(f"SMTP Server (default: smtp.office365.com): ").strip() or "smtp.office365.com"
        smtp_port = input(f"SMTP Port (default: 587): ").strip() or "587"
        email_user = input(f"Email User (default: support@katalystvc.com): ").strip() or "support@katalystvc.com"
        
        print(f"Enter the password for {email_user}:")
        print("Note: For Microsoft accounts, you may need to use an App Password.")
        email_password = getpass.getpass("Email Password: ")
        
        internal_email = input(f"Internal notification email (default: {email_user}): ").strip() or email_user
        ...
        with open(env_file, 'w') as f:
            ...
            f.write(f"INTERNAL_EMAIL={internal_email}\n")
        
        # Set restrictive permissions on .env file
        os.chmod(env_file, 0o600)  # Read/write for owner only
        
        print(f"\nCredentials saved securely to {env_file}")
        print("File permissions set to 600 (owner read/write only)")
        print("IMPORTANT: Add .env to your .gitignore file to prevent accidental commits!")
...
if __name__ == '__main__':
    print("KatalystVC Excel API Microservice (Secure Version) starting...")
    print(f"Excel file: {EXCEL_FILE}")
    
    # Setup credentials if needed
    setup_credentials()
    
    # Reload environment variables after potential setup
    global EMAIL_PASSWORD, EMAIL_USER, SMTP_SERVER, SMTP_PORT, INTERNAL_EMAIL
    ...
    print(f"Email configured: {'Yes' if EMAIL_PASSWORD else 'No'}")
